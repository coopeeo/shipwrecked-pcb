...
class Ecdsa:
    ...
    @classmethod
    def sign(cls, byteMessage, privateKey, hashfunc=sha256):
        byteMessage = hashfunc(byteMessage).digest()
    ...
    @classmethod
    def verify(cls, byteMessage, signature, publicKey, hashfunc=sha256):
        byteMessage = hashfunc(byteMessage).digest()
        numberMessage = numberFromByteString(byteMessage)
